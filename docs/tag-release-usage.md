...
Release tagging script for `delphi-inspect`.
...
- [ ] `$ToolVersion` in `source/delphi-inspect.ps1` has been updated to `X.Y.Z`
...
1. `$ToolVersion` in `source/delphi-inspect.ps1` matches the `-Version` argument
...
delphi-inspect  tag-release
===========================
  Version : 1.0.0
  Tag     : v1.0.0
  Repo    : C:\dev\delphi-inspect
...
```text
FAIL  $ToolVersion in delphi-inspect.ps1 is '1.0.0' but -Version arg is '1.1.0'.
      Update $ToolVersion in the script and commit before tagging.
```

Update `$ToolVersion = '1.1.0'` in `source/delphi-inspect.ps1`, commit, push, then re-run.
...
```text
FAIL  Must be on 'main' branch to tag a release (currently on 'feature/add-locate').
