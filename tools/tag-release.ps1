...
# Creates and pushes a vX.Y.Z release tag for delphi-inspect.
...
#   - $ToolVersion in delphi-inspect.ps1 matches the Version argument
...
Write-Host ""
Write-Host "delphi-inspect  tag-release" -ForegroundColor White
...
if ($scriptContent -notmatch '\$ToolVersion\s*=\s*''([^'']+)''') {
  Fail "Could not find '`$ToolVersion = ''...''' in delphi-inspect.ps1."
}

$scriptVersion = $Matches[1]
if ($scriptVersion -ne $Version) {
  Fail "`$ToolVersion in delphi-inspect.ps1 is '$scriptVersion' but -Version arg is '$Version'.`n       Update `$ToolVersion in the script and commit before tagging."
}
